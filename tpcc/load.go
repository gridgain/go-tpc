...
c_discount, c_balance, c_ytd_payment, c_payment_cnt, c_delivery_cnt, c_data) VALUES `
	...
	for i := 0; i < customerPerDistrict; i++ {
		...
		var cSince interface{}
		if w.cfg.Driver == "odbc" {
			cSince = sink.SQLTimestamp{Val: w.initLoadTime, Valid: true}
		} else {
...
func (w *Workloader) loadHistory(ctx context.Context, warehouse int, district int) error {
	...
	var hint string
	if w.cfg.Driver == "odbc" {
		hint = `INSERT INTO history (h_c_id, h_c_d_id, h_c_w_id, h_d_id, h_w_id, h_date, h_amount, h_data, h_id) VALUES `
	} else {
	...
	for i := 0; i < customerPerDistrict; i++ {
		...
		var hDate interface{}
		if w.cfg.Driver == "odbc" {
			hDate = sink.SQLTimestamp{Val: w.initLoadTime, Valid: true}
		} else {
		...
		if w.cfg.Driver == "odbc" {
			...
			if err := l.WriteRow(ctx,
				hCID, hCDID, hCWID, hDID, hWID, hDate, hAmount, hData, hID,
			); err != nil {
				return err
			}
		} else {
...
o_carrier_id, o_ol_cnt, o_all_local) VALUES `
	...
	for i := 0; i < orderPerDistrict; i++ {
		...
		if w.cfg.Driver == "odbc" {
			oEntryD = sink.SQLTimestamp{Val: w.initLoadTime, Valid: true}
		} else {
...
ol_i_id, ol_supply_w_id, ol_delivery_d, ol_quantity, ol_amount, ol_dist_info) VALUES `
	...
	for i := 0; i < orderPerDistrict; i++ {
		for j := 0; j < olCnts[i]; j++ {
			...
			if olOID < 2101 {
				if w.cfg.Driver == "odbc" {
					olDeliveryD = sink.SQLTimestamp{Val: w.initLoadTime, Valid: true}
				} else {
				...
				if w.cfg.Driver == "odbc" {
					olDeliveryD = sink.SQLTimestamp{Valid: false}
				} else {
					olDeliveryD = sql.NullString{}
				}
